...
def test_houses_pout(world):
    # house_data = world.sims['HouseholdSim-0'].Houses
    house_data = world.get_data(HOUSES, 'P_out')
...
def create_scenario(world):
    ...
    node_simulator = world.start('NodeSimulator', sim_start=START, eid_prefix='edgeNode', data_file=PV_DATA, grid_name=GRID_NAME)
    ...
    node_simulator_nodes = node_simulator.Node.create(1, init_val='1')
...
def connect_node_to_grid(world, nodes, buses):
    ...
    node_data = world.get_data(nodes, 'node_id')
    for node in nodes:
